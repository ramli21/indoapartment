...
@section('content')
    <section class="pt-24 pb-12 bg-slate-50 min-h-screen">
        <div class="max-w-2xl mx-auto px-4 sm:px-6">
            <div class="mb-6">
                <h1 class="text-2xl font-semibold">Tambah Payment Config</h1>
            </div>
            ...
            <div class="bg-white rounded-2xl border border-slate-100 shadow-sm p-6">
                <form action="{{ route('admin.payment-configs.store') }}" method="POST">
                    @csrf
                    <div class="grid grid-cols-1 gap-4">
                        <label class="block">
                            <div class="text-sm text-slate-600 mb-1">Provider Name</div>
                            <input type="text" name="provider_name" value="doku"
                                class="w-full px-3 py-2 border rounded" />
                        </label>

                        <label class="block">
                            <div class="text-sm text-slate-600 mb-1">Merchant ID</div>
                            <input type="text" name="merchant_id" value="" class="w-full px-3 py-2 border rounded"
                                required />
                        </label>

                        <label class="block">
                            <div class="text-sm text-slate-600 mb-1">Client ID</div>
                            <input type="text" name="client_id" value="" class="w-full px-3 py-2 border rounded"
                                required />
                        </label>

                        <label class="block">
                            <div class="text-sm text-slate-600 mb-1">Shared Key</div>
                            <input type="text" name="shared_key" value="" class="w-full px-3 py-2 border rounded"
                                required />
                        </label>
