...
@section('content')
    <section class="pt-24 pb-12 min-h-screen">
        <div class="max-w-4xl px-4 sm:px-6">
            ...
            @if ($statusFilter)
                <div
                    class="mb-6 p-4 bg-blue-50 border border-blue-200 rounded-xl flex items-center gap-3 text-blue-700 animate-fadeIn">
                    <i data-lucide="filter" class="w-5 h-5 shrink-0"></i>
                    <span class="text-sm font-medium w-5/6">Menampilkan room dengan status:
                        <strong>{{ $statusFilter }}</strong></span>

                    <a href="{{ route('admin.all.rooms') }}"
                        class="text-sm text-red-600 hover:text-red-800 text-end w-1/6">Clear
                        Filter</a>
                </div>
            @endif
            ...
            <div class="grid grid-cols-2 lg:grid-cols-5 gap-5 mb-8">
                ...
                <a href="{{ route('admin.all.rooms', ['status' => 'Tersedia']) }}"
                ...
                <a href="{{ route('admin.all.rooms', ['status' => 'Terisi']) }}"
                    class="bg-white rounded-2xl border border-slate-100 p-5 shadow-sm hover:shadow-md transition-shadow">
                    <div class="flex items-center justify-between mb-3">
                ...
                <a href="{{ route('admin.all.rooms', ['status' => 'Pending']) }}"
                    class="bg-white rounded-2xl border border-slate-100 p-5 shadow-sm hover:shadow-md transition-shadow">
                    <div class="flex items-center justify-between mb-3">
                    ...
                    <div class="text-2xl font-bold text-slate-800">{{ $pending }}</div>
                    <div class="text-xs text-slate-500 mt-1">Menunggu konfirmasi</div>
                </a>

                <a href="{{ route('admin.all.rooms', ['status' => 'Perawatan']) }}"
                    class="bg-white rounded-2xl border border-slate-100 p-5 shadow-sm hover:shadow-md transition-shadow">
                    <div class="flex items-center justify-between mb-3">
                                    ...
                                    <tr class="hover:bg-slate-50/50 transition-colors group">
                                        ...
                                        <td class="px-5 py-4 whitespace-nowrap">
                                            <div>
                                                ...
                                                <div class="text-xs text-slate-400 mt-0.5 line-clamp-1">
                                                    {{ Str::limit($room->apartment->nama, 40) }}</div>
                                            </div>
                                        </td>
                                        ...
                                        <td class="px-5 py-4 whitespace-nowrap">
                                            <div class="flex items-center justify-end gap-1.5">
                                                ...
                                                <form
                                                    action="{{ route('admin.apartments.rooms.destroy', [$room->apartment_id, $room->id]) }}"
                                                    method="POST"
